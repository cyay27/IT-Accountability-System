...
export const useSoftwareInventoryRecords = () => {
  ...
  const loadRecords = useCallback(async () => {
    ...
    if (useApiMode) {
      ...
      try {
        const response = await getApiCollectionRecords<SoftwareInventoryRecord>(COLLECTION_NAME, {
          orderBy: "updatedAt",
          direction: "desc",
          limit: 100
        });
        ...
        setError("Access denied or unable to load software records from API.");
        setRecords([]);
      } finally {
        setLoading(false);
      }
